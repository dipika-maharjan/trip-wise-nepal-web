...
export class BookingService {
    async createBooking(data: CreateBookingDTO, userId: string) {
        ...
        if (bookingLocks[lockKey]) {
            throw new HttpError(409, "Another booking is being processed for these dates. Please try again in a moment.");
        }
        ...
        try {
            const accommodation = await accommodationRepository.getAccommodationById(data.accommodationId);
            ...
            const extras = await bookingExtraRepository.getExtrasByBookingId(String(newBooking._id));
    ...
    async getBookingById(id: string) {
        try {
            ...
            const extras = await bookingExtraRepository.getExtrasByBookingId(String(booking._id));
    ...
    async cancelBooking(id: string) {
        try {
            ...
            if (booking.checkIn && new Date(booking.checkIn) <= today) {
                throw new HttpError(400, "Cannot cancel a booking on or after check-in date");
            }
